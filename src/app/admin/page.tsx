  ...
  const handleTogglePermission = async (
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (selectedUser) {
          setSelectedUser(
            data.users.find((u: any) => u.id === selectedUser.id),
          );
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-surface-50 to-surface-100">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {users.map((u) => (
                      <tr key={u.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${roleColors[u.role] || "bg-gray-100 text-gray-700"}`}
                          >
      ...
      <Modal
        ...
        title={`Права: ${selectedUser?.name}`}
        size="md"
      >
        {selectedUser && (
          <div className="space-y-4">
            <p className="text-sm text-ink-muted">
              Управление гранулярными правами пользователя{" "}
              <strong>{selectedUser.name}</strong> (
              {selectedUser.role === "PM" ? "PM" : "Разработчик"})
            </p>

            <div className="space-y-3">
              {AVAILABLE_PERMISSIONS.map((perm) => {
                const hasPerm = (selectedUser.permissions || []).includes(
                  perm.key,
                );
                return (
                  <div
