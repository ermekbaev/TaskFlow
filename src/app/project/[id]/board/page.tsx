...
const ProjectBoard: React.FC = () => {
  ...
  const [gitRepo, setGitRepo] = useState(
    "https://github.com/company/project-app",
  );
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                <div className="min-w-[2.5rem] h-10 px-2 bg-primary-100 rounded-lg flex items-center justify-center">
                  <span className="text-primary-700 font-bold text-sm truncate max-w-[5rem]">
                    {project.key}
                  </span>
                </div>
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-ink to-primary-600 bg-clip-text text-transparent">
                    {project.name}
                  </h1>
                ...
                <div className="flex items-center flex-wrap gap-2 text-sm text-ink-muted">
                  ...
                  {selectedTask.isRecurring && (
                    <span className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-700">
                      ...
                      {selectedTask.recurrencePattern === "daily"
                        ? "Ежедневно"
                        : selectedTask.recurrencePattern === "weekly"
                          ? "Еженедельно"
                          : selectedTask.recurrencePattern === "biweekly"
                            ? "Раз в 2 нед."
                            : selectedTask.recurrencePattern === "monthly"
                              ? "Ежемесячно"
                              : ""}
                    </span>
                  )}
                      ...
                      {selectedTask.initiatorName && (
                        <div>
                          ...
                          <div className="text-sm text-ink-muted space-y-0.5">
                            ...
                            {selectedTask.initiatorEmail && (
                              <p>{selectedTask.initiatorEmail}</p>
                            )}
                            {selectedTask.initiatorPosition && (
                              <p>{selectedTask.initiatorPosition}</p>
                            )}
                          </div>
                        </div>
                      )}
                      {selectedTask.curatorName && (
                        <div>
                          ...
                          <div className="text-sm text-ink-muted space-y-0.5">
                            ...
                            {selectedTask.curatorEmail && (
                              <p>{selectedTask.curatorEmail}</p>
                            )}
                            {selectedTask.curatorPosition && (
                              <p>{selectedTask.curatorPosition}</p>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${
                          selectedTask.acceptanceStatus === "accepted"
                            ? "bg-green-100 text-green-700"
                            : selectedTask.acceptanceStatus === "rejected"
                              ? "bg-red-100 text-red-700"
                              : selectedTask.acceptanceStatus === "in_review"
                                ? "bg-amber-100 text-amber-700"
                  ...
                  {parseCustomDates(selectedTask.customDates).length > 0 && (
                    <div>
                      ...
                      <div className="space-y-1 text-sm">
                        {parseCustomDates(selectedTask.customDates).map(
                          (cd: any, i: number) => (
                            <div
                              key={i}
                            ...
                            >
                              <span>{cd.name}:</span>
                              <span>
                                {new Date(cd.date).toLocaleDateString("ru-RU")}
                              </span>
                            </div>
                          ),
                        )}
              ...
              {parseLabels(selectedTask.labels).length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {parseLabels(selectedTask.labels).map(
                      (label: string, index: number) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-mint-100 text-mint-800"
                        >
                          {label}
                        </span>
                      ),
                ...
                <div className="space-y-4">
                  <FileUpload
                    ...
                    attachments={taskAttachments.filter(
                      (a) => a.category === "labor_estimate",
                    )}
                    onUpload={(att) =>
                      setTaskAttachments([att, ...taskAttachments])
                    }
                    onDelete={(delId) =>
                      setTaskAttachments(
                        taskAttachments.filter((a) => a.id !== delId),
                      )
                    }
            ...
            <div className="border-t pt-4 mt-4 flex flex-wrap gap-3">
              ...
              {(isManager || selectedTask.assigneeId === currentUser.id) && (
                <Button
                  ...
                  onClick={async () => {
                    if (
                      !confirm("Удалить эту задачу? Это действие необратимо.")
                    )
                      return;
                    try {
                      const res = await fetch(`/api/tasks/${selectedTask.id}`, {
