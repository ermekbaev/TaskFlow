...
const EventViewEditModal: React.FC<EventViewEditModalProps> = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isEditing ? "Редактировать событие" : event.title}
      size="md"
    >
      {!isEditing ? (
        // View Mode
        <div className="space-y-4">
          <div
            className={`p-4 rounded-lg ${event.color || "bg-primary-100 text-primary-700"}`}
          >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="text-ink mt-1">
                {format(new Date(event.endDate), "d MMMM yyyy", { locale: ru })}
              </p>
            </div>
            <div>
              <label className="text-sm font-medium text-ink-muted">
          ...
          {event.reminderTime && (
            <div>
              ...
              <p className="text-ink mt-1">
                За{" "}
                {event.reminderTime === "1440"
                  ? "1 день"
                  : `${event.reminderTime} мин`}
              </p>
            </div>
          )}
          ...
          <div className="border-t border-surface-200 pt-4">
            <FileUpload
              ...
              onDelete={(id) => {
                setEventAttachments(
                  eventAttachments.filter((a) => a.id !== id),
                );
              }}
              label="Вложения"
            />
