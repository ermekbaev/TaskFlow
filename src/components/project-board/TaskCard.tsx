...
const TaskCard: React.FC<TaskCardProps> = ({ task, onDragStart, canEdit, onClick }) => {
  ...
  return (
    <div
      ...
      className={`p-3 rounded-lg border-l-4 cursor-pointer shadow-sm hover:shadow-md transition-all duration-200 group ${getPriorityColor(task.priority)}`}
      onClick={onClick}
    >
      {/* Шапка: ключ и меню */}
      ...
      {/* Заголовок */}
      <h4 className="font-medium text-gray-900 text-sm mb-1.5 line-clamp-2 group-hover:text-blue-600 transition-colors leading-snug">{task.title}</h4>
